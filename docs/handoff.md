...
- **לפני אימות מקומי:** לסגור את *כל* מופעי `http.server` דרך PowerShell — מופעים
  יתומים מסשנים קודמים מגישים קבצים ישנים.
- **"השינוי ב-CSS לא נתפס" — הסיבה האמיתית:** הקאש של הדפדפן על תגית ה-`<link>`.
  `fetch` מחזיר את הקובץ החדש בזמן שהגיליון שנפרסר ישן. רענון רגיל לא מספיק —
  צריך להחליף את תגית ה-link ב-`href` עם `?bust=<timestamp>` ולחכות ל-`onload`.
